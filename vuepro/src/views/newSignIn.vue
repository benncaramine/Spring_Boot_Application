...
export default {
  ...
  setup() {
    ...
    const login = () => {
      axios.post('http://localhost:8081/user/login', { email: email.value, password: password.value })
        .then(response => {
          // Authentification réussie
          const { token, role, username, email } = response.data; // Récupérer le token et le rôle depuis la réponse
		  ...
		  localStorage.setItem('isLoggedIn', true); // Marquez l'utilisateur comme connecté
          ...
          if (role === 'admin') {
            ...
            // Rediriger l'utilisateur vers sa page
            window.location.href = '/user/home';
          } else if (role === 'employe') {
            // Rediriger l'employé vers sa page
            window.location.href = '/employe/home';
          } else {
            // Gérer d'autres cas de rôle ici
            // Par exemple, rediriger vers une page par défaut si le rôle n'est pas reconnu
