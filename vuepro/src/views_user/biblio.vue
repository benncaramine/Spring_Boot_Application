          ...
          <table class="table">
            ...
            <tbody>
              <tr v-for="(item, index) in items" :key="index" :class="{ 'table-active': index === selectedIndex }">
                  ...
                  <td :class="{ 'text-success': item.empruntable, 'text-danger': !item.empruntable }">{{ item.empruntable }}</td>
                  <td>
                    <button @click="consulter(item.document_id)" class="btn btn-primary">Emprunter</button>
...
axios.post(`http://localhost:8081/api/emprunts/add`, data, config)
  .then(response => {
